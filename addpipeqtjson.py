...
def insert_pipe(pipe):
    pipe_data = json.dumps(pipe.__dict__)
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS pipes (id INTEGER PRIMARY KEY, data TEXT)''')
    c.execute("INSERT INTO pipes (data) VALUES (?)", (pipe_data,))
    conn.commit()
    conn.close()
